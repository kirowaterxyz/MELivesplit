...
void* FindPattern(char* module, const char* pattern, const char* mask) {
	...
	const char* base = static_cast<const char*>(moduleInfo.lpBaseOfDll);
...
// Assuming Disassemble and Memcpy functions are properly defined elsewhere

void UnTrampolineHook(void *src, void **gate) {
	char disassembled[0xFF];

	if (src == nullptr || gate == nullptr || *gate == nullptr) {
		std::cerr << "Error: Invalid pointers provided to UnTrampolineHook." << std::endl;
		return;
	}

	MEMORY_BASIC_INFORMATION mbi;
	if (!VirtualQuery(*gate, &mbi, sizeof(mbi))) {
		std::cerr << "Error: VirtualQuery failed with code " << GetLastError() << std::endl;
		return;
	}

	if (!(mbi.Type & MEM_PRIVATE)) {
		std::cerr << "Error: Memory at gate is not private allocation." << std::endl;
		return;
	}
	...
	int relative = -1;
	while (size < 5) {
		unsigned char byte = *(unsigned char *)((uintptr_t)*gate + size);
		if (byte == 0xE8 || byte == 0xE9) {
			relative = size;
		}

		int disassembledSize = Disassemble((unsigned char *)((uintptr_t)*gate + size), 20, size, disassembled);
		if (disassembledSize == 0) {
			std::cerr << "Error: Disassembly failed at offset " << size << std::endl;
			return;
		}

		size += disassembledSize;
	}

	if (relative != -1) {
		int disassembledSize = Disassemble((unsigned char *)((uintptr_t)*gate + relative), 20, (uintptr_t)*gate, disassembled);
		if (disassembledSize == 0) {
			std::cerr << "Error: Disassembly failed at relative offset " << relative << std::endl;
			return;
		}

		int addr;
		if (sscanf(strchr(disassembled, ' ') + 1, "0x%x", &addr) != 1) {
			std::cerr << "Error: Failed to parse address from disassembled instruction." << std::endl;
			return;
		}

		*(int *)(((uintptr_t)*gate) + relative + 1) = addr - (((uintptr_t)src) + relative) - 5;
	}

	Memcpy(src, *gate, size);

	if (!VirtualFree(*gate, 0, MEM_RELEASE)) {
		std::cerr << "Error: VirtualFree failed with code " << GetLastError() << std::endl;
		return;
	}
...
unsigned int *GetD3D9Exports() {
	...
	//return *(unsigned int **)((unsigned int)FindPattern((void *)base, 0x128000, "\xC7\x06\x38\x26\x79\x64\x89\x86\x28\x32\x00\x00\x89\x86", "xxxxxx????xx????xx") + 2);


	HMODULE hMod = GetModuleHandleA("MirrorsEdge.exe");
	if (hMod == nullptr) {
		std::cerr << "Failed to get the module handle for MirrorsEdge.exe" << std::endl;
		return 0;
	}


	const char* pattern = "\x89\x0D\x00\x00\x00\x00\xB9\x00\x00\x00\x00\xFF";
	const char* mask = "xx????x????x";

	void* result = FindPattern("MirrorsEdge.exe", pattern, mask);

	std::cout << "Base address of Mirror's Edge: " << hMod << std::endl;

	if (result) {
		std::cout << "Pattern found at: " << result << std::endl;
	}
	else {
